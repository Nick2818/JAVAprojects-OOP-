...
public class Bank {
	...
	/**
	 * Runs the program by initialising and managing, bank accounts and customers.
	 */
	public void run() {
		...
		//from checking
		System.out.println();
		System.out.println("Amount (decimal) to withdrwa from your checkings account?");
		amount = scanner.nextDouble(); //get next token (double)
		try {
		...
		//from savings
		System.out.println();
		System.out.println("Amount (decimal) to withdrwa from your savings account?");
		amount = scanner.nextDouble(); //get next token (double)
		try {
			savingsAccount.withdraw(amount); //withdraw from saving
		} catch (Exception e) {
			//e.printStackTrace();
			//prints custom error message from withdraw method
			System.out.println(e.getMessage());			
