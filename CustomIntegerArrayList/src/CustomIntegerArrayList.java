...
public class CustomIntegerArrayList {
	...
	public ArrayList<Integer> splice(int index, int num) {
        ...
        int numToRemove = Math.min(num, this.arr.size() - index);

        for (int i = index; i < index + numToRemove; i++) {
            removedElements.add(this.arr.get(i));
        }
        this.arr.subList(index, index + numToRemove).clear();
	...
	public ArrayList<Integer> splice(int index, int num, int[] otherArray) {
		if (index < 0 || index >= arr.size() || num == 0) {
            // invalid input, don't remove or insert anything
            return new ArrayList<Integer>();
        }

        // determine how many elements to remove (up to the end of the list)
        int numToRemove = Math.min(num, arr.size() - index);

        // remove the elements and store them in a new ArrayList
